...
description: >-
  ...
  LCD, spindle_control и ui_buttons; когда пользователь упоминает делительную головку,
  divider, A/B/R/COU, деление окружности, части деления, отклонение угла или сброс угла.
---
...
Вторая строка: A 001 B 001 R-000.00
Третья строка: COU -0000000:0000
Четвертая строка стандартно

Параметры: 
Текущий угол поворота сбрасывется в 0 нажатием Select
A количество частей деления, изменияется кнопками UP/DW, по умолчанию 1, максимум 999
...
4. Select в режиме 6 — **сброс опорного угла**; long Select — меню EEPROM (`els-display-menu`).
5. Строки 0 и 3 — как главный экран; строки 1–2 — поля divider.
...
| **A** | Up / Down | 1…999 | 1 |
| **B** | Left / Right | 1…A | 1 |
| **Сброс угла** | **Select** (короткое) | — | `zero_count = spindle_get_count()` |
...
- Сброс: `mode_divider_zero_reset()` → запомнить `zero_count`, не обнулять B.
...
- [ ] Стр. 1: A 001 B 001 R±000.00
- [ ] Стр. 2: COU ±0000000:0000
- [ ] Стр. 3: coords + markers
- [ ] A: Up/Down 1…99; B: L/R 1…A, wrap; B≤A при смене A
- [ ] Select → zero_count = spindle_get_count()
