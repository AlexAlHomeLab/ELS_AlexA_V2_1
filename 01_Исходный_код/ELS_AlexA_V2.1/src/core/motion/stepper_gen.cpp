...
/* SPS для backlash_arm: при РГИ не от номинала селектора шага (только BlSp/BlMn) */
static uint32_t motion_backlash_arm_sps(uint8_t axis, float feed_mm_min, uint8_t mpg) {
    float feed;

    if (mpg) {
        feed = config_backlash_comp_speed_mm_min(axis, (float)config_backlash_get_auto_speed());
    } else {
        feed = config_backlash_comp_speed_mm_min(axis, feed_mm_min);
    }
