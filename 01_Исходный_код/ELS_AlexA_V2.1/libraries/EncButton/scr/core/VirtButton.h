...
class VirtButton {
#ifdef __AVR__
    typedef void (*ActionHandler)();//вписано
#else
    typedef std::function<void()> ActionHandler;//вписано
#endif

   public:
	uint8_t mpinid = 0; //вписано
	uint8_t getid(){ 	//вписано 
		return mpinid;
	}
	//вписано id кнопки для обратного вызова
